...
export function Header() {
  ...
  return (
    <motion.header
      ...
      className="fixed top-0 left-0 right-0 z-50 bg-[var(--background)]/90 backdrop-blur-md border-b border-white/5"
    >
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="md:hidden fixed top-16 left-0 right-0 bottom-0 z-40 bg-[var(--background)] flex flex-col"
          >
            <ul className="px-6 py-8 flex flex-col gap-4 overflow-auto">
              {navAnchors.map(({ href, key }) => (
                <li key={href}>
                  <a
                    href={href}
              ...
              <li className="pt-6 flex flex-wrap gap-4">
                {socialLinks.map((link) => (
                  <a
                    key={link.icon}
                    href={link.href}
